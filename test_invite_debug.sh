...
# Test específico para debuggear INVITE
PORT=6668
PASS="testpass123"

echo "=== TEST DEBUG INVITE ==="
...
if read -t 2 -r line <&3; then
    ...
    if echo "$line" | grep -q "401"; then
        ...
        echo "❌ DESCONOCIDO - Devuelve código inesperado"
    fi
else
...
if read -t 2 -r line <&3; then
    ...
    if echo "$line" | grep -q "403"; then
        ...
        echo "❌ INCORRECTO - Devuelve código inesperado"
    fi
else
    echo "❌ FAIL - No se recibió respuesta"
fi
...
echo "=== ANÁLISIS ==="
echo "Según el estándar IRC:"
echo "- 401 ERR_NOSUCHNICK: Usuario no existe"
echo "- 403 ERR_NOSUCHCHANNEL: Canal no existe"
echo ""
echo "Para INVITE:"
echo "- Si el usuario no existe: 401"
echo "- Si el canal no existe: 403"
echo ""
