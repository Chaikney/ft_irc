...
#include "include/ReplyEnums.hpp"
...
void	Server::handleJoin(Message *msg, User *usr)
{
    ...
    {
        usr->addChannel(chan);
		std::cout << "=== User " << usr->getNick() << " (FD:" << usr->getFD() << ") successfully joined " << chan << " ===" << std::endl;
		...
		// Send names list (REQUIRED for client to show the channel)
